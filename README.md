...
| `pricing.html` | Pricing & Policies page: $25/hour, car options, same-day payment, requirements checklist, FAQ |
...
| Mark a lesson done | Set **Status** to `Completed` or `No-show` (for your own records) |
...
## Part 3 — Put the website online with GitHub Pages

### Step 1: Personalize the site

Search the files for `REPLACE` to find every placeholder:

- **Phone, email and service area:** the footer of `index.html` and `pricing.html`
  (currently `(555) 010-0000`, `hello@example.com` and "Your City").
...
- **Payment methods (optional):** in `pricing.html`, list what Shun accepts
  (cash, Zelle, Venmo…).

If you ever change the price or lesson length, update it in **both** places:
`RATE_PER_HOUR` / `SLOT_MINUTES` in `script.js` and in `Code.gs`, plus the prices
written on the two HTML pages.
...
2. Drag in `index.html`, `pricing.html`, `styles.css`, `script.js`, `favicon.svg`
   and `README.md`. These must sit at the **top level** of the repository, not
